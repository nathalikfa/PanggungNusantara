...
<head>
    ...
    <title>Search Results - Panggung Nusantara</title>
...
<body class="font-sans antialiased bg-gray-100">
    ...
    <section class="bg-gradient-to-r from-purple-700 to-pink-500 text-white py-20 mt-16">
        <div class="container mx-auto px-6 text-center">
            <h1 class="text-6xl font-bold">Search Results</h1>
            <p class="text-lg mt-4">Artist related to your search: <strong>{{ $artist->name }}</strong></p>
        </div>
    </section>

    <section class="py-16">
        <div class="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
            <!-- Concert Schedule -->
            <div class="bg-gray-50 p-6 rounded-lg shadow-lg">
                <h2 class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-700 to-pink-500 mb-6">
                    Concert Schedule
                </h2>
                <div class="space-y-4">
                    @forelse ($artist->concerts as $concert)
                    <div class="bg-white border border-gray-200 rounded-lg p-4 flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-bold text-gray-800">{{ $concert->name }}</h3>
                            <p class="text-gray-600">Date: {{ \Carbon\Carbon::parse($concert->date)->format('d F Y') }}</p>
                            <p class="text-gray-600">Location: {{ $concert->location }}</p>
                            <p class="text-gray-600">Price Range: Rp.{{ number_format($concert->min_price, 2) }} - Rp.{{ number_format($concert->max_price, 2) }}</p>
                        </div>
                        @auth
                        <a href="{{ route('payment.showSeats', ['concertId' => $concert->id]) }}"
                            class="bg-pink-500 text-white px-6 py-2 rounded-full hover:bg-pink-600">
                            Get Ticket
                        </a>
                        @else
                        <a href="{{ route('login') }}"
                    ...
                    @empty
                    <p class="text-gray-600">No concerts available for this artist.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="flex flex-col items-start space-y-8">
                ...
                <!-- Artist Image -->
                <div class="w-full">
                    <img src="{{ $artist->image ?? '/img/default_artist.jpg' }}" alt="{{ $artist->name }}" class="w-full rounded-lg shadow-lg">
                </div>

                <!-- Biography -->
                <div>
                    <h2 class="text-3xl font-bold text-gray-800 mb-6">{{ $artist->name }}</h2>
                    <p class="text-gray-700 leading-relaxed">
                        {{ $artist->bio ?? 'Biography not available.' }}
                    </p>

                    <!-- Genre and Established Date -->
                    <div class="mt-6">
                        <p class="text-lg text-gray-800"><strong>Genre:</strong> {{ $artist->genre ?? 'Unknown' }}</p>
                        <p class="text-lg text-gray-800"><strong>Established:</strong> {{ $artist->date_of_birth ?? 'Unknown' }}</p>
                    </div>
                </div>
                @else
                <p class="text-gray-600">Artist data not available.</p>
                @endif
            </div>
